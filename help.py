...
class Help(commands.Cog):
    ...
    async def help(self, ctx, *cog):
        if not cog:
            ...
            if len(cog) > 1:
                embed = discord.Embed(colour= 0x7a5901,title='Error', description='Too many Cogs!')
                ...
                if not found:
                    for x in self.client.cogs:
                        ...
                        found = True
                    if not found:
                        embed = discord.Embed(title= 'Error', description= f'How do you even use {cog[0]}?', colour=0x7a5901)
